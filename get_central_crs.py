...
ks=['江青','云鹤','文元','洪文','春桥']
if not os.path.exists(p1:='中央文革CCRD文库全集（标题提及）'):
    os.mkdir(p1)
if not os.path.exists(p2:='中央文革CCRD文库全集（内容提及）'):
    os.mkdir(p2)
...
ks=['远新']
if not os.path.exists(p1:='毛远新CCRD文库全集（标题提及）'):
    os.mkdir(p1)
if not os.path.exists(p2:='毛远新CCRD文库全集（内容提及）'):
    os.mkdir(p2)
for a in os.walk('HTML'):
    a[2].sort()
    lf=[b for b in a[2]if re.search(r'\d{6}',b)]
